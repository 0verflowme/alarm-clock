...
function createNewAlarmDOM() {
  ...
  if (
    ...
    Number(userMinutes.value) < 0 ||
    Number(userMinutes.value) > 60 ||
  ...
  alarmItem.classList.add("rounded-3");

  // elements inside alarmItems

  var logo = document.createElement("img");
  logo.setAttribute("src", "img/icons8-retro-alarm-clock-24.png");
  ...
  alarmTime.innerHTML =
    userHours.value + ":" + userMinutes.value + ":" + userSeconds.value;
  ...
  delButton.setAttribute("src", "img/icons8-delete-26.png");

  delButton.onclick = function () {
    // first remove the data from alarmsObj
    const currentAlarmTime =
      this.parentNode.childNodes[1].childNodes[0].innerHTML;
